...
Este repositorio contiene **únicamente documentación** (Markdown + PDF). No existe test runner, build pipeline ni comandos de desarrollo.

| Tarea | Comando |
|-------|---------|
| Vista previa Mermaid | Abrir `.md` en VS Code con extensión Mermaid Preview, o usar mermaid.live |
| Validar Markdown | `npx markdownlint-cli docs/**/*.md` (si se instala localmente) |
| Test suite completa | N/A |
| Narrowed test run | N/A |

> En `/refacil:verify` y `/refacil:test`, el "test" se interpreta como revisión de consistencia del documento: coherencia entre diagramas, secciones y requisitos del caso de estudio.
